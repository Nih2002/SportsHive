  ...
  <div class="h-96 bg-cover bg-center" style="background-image: url('../images/sportsback1.jpg');">
    <body class="bg-gray-100">
        <div class="flex flex-col md:flex-row items-center justify-center h-96">
            ...
            <img src="../images/football.jpg" alt="Jumping athlete" class="w-full h-80 object-cover">
            </div>
        </div>
  </div>
...
<div class="col-span-3 max-w-7xl mx-auto py-12 px-4">
        <div class="max-w-7xl mx-auto py-12 px-4 ">
          ...
          $sql = "SELECT name, price, image_url, sizes, colors, stock_status, description FROM football WHERE item_type = 'Shoes'";
          ...
          if ($result->num_rows > 0) {
              while ($row = $result->fetch_assoc()) {
                  ...
                  echo '<button class="add-to-cart bg-red-500 text-white text-sm font-medium py-2 px-4 rounded-full hover:bg-red-600 transition duration-300 shadow-md" data-name="' . $name . '" data-price="' . $price . '">ADD TO CART</button>';
                  echo '<button class="add-to-wishlist flex items-center space-x-2 text-sm font-medium text-red-500 transition duration-300" data-name="' . $name . '" data-price="' . $price . '" data-image="' . htmlspecialchars($main_image, ENT_QUOTES, 'UTF-8') . '">';
...
<div class="text-center mb-12 ">
            <h1 class="text-4xl font-bold text-gray-800 mt-8">OUR FOOTBALL BALLS</h1>
            </div>
          ...
          // Fetch products from football table where item_type is 'Shoes'
          $sql = "SELECT name, price, image_url, sizes, colors, stock_status, description FROM football WHERE item_type = 'Balls'";
          $result = $conn->query($sql);
