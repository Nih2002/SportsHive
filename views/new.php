...
<div class="col-span-3 max-w-7xl mx-auto py-12 px-4">
        <div class="max-w-7xl mx-auto py-12 px-4 ">
            ...
            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                ...
                <button class="see-more bg-blue-500 text-white text-sm font-medium py-2 px-4 rounded-full hover:bg-blue-600 transition-colors duration-300" data-id="76">
                    See More
                    </button>
                </div>
                </a>
            </div>
            <div id="item-modal-76" class="fixed inset-0 z-50 hidden bg-gray-900 bg-opacity-50 flex justify-center items-center">
    <div class="bg-white rounded-lg shadow-lg p-6 w-11/12 md:w-3/4 lg:w-1/2">
        <button id="close-modal-76" class="absolute top-4 right-4 text-gray-500 hover:text-gray-700">&times;</button>
        <div id="modal-content-76">
            <!-- Dynamic content for item 76 will be inserted here -->
        </div>
    </div>
</div>

<div id="item-modal-77" class="fixed inset-0 z-50 hidden bg-gray-900 bg-opacity-50 flex justify-center items-center">
    <div class="bg-white rounded-lg shadow-lg p-6 w-11/12 md:w-3/4 lg:w-1/2">
        <button id="close-modal-77" class="absolute top-4 right-4 text-gray-500 hover:text-gray-700">&times;</button>
        <div id="modal-content-77">
            <!-- Dynamic content for item 77 will be inserted here -->
        </div>
    </div>
</div>
...
<script>
    const catalogData = [
        {
            ...
            name: "Running Shoes",
            price: 5200,
            description: "Lightweight | EVA | Ideal for Trail Running",
            sizes: ["4", "5", "6"],
            colors: ["blue", "gray", "black"],
            images: ["../images/cshose1.png", "../images/cshose2.png", "../images/cshose3.png"]
        },
        {
            id: 77,
            name: "ASS SMACKER Cricket Shoe",
            price: 13500,
            description: "ASS SMACKER Cricket Shoe",
            sizes: ["4", "5", "6"],
            colors: ["blue", "gray", "black"],
            images: ["../images/cshose1.png", "../images/cshose2.png", "../images/cshose3.png"]
        }
    ...
    document.querySelectorAll(".see-more").forEach(button => {
        button.addEventListener("click", () => {
            ...
            const item = catalogData.find(product => product.id === itemId);

            if (item) {
                const modalId = `#item-modal-${item.id}`;
                const modalContentId = `#modal-content-${item.id}`;
                const modal = document.querySelector(modalId);
                const modalContent = `
                    <h2 class="text-xl font-bold">${item.name}</h2>
                    <p class="text-gray-600">${item.description}</p>
                    <p class="text-yellow-500 text-lg font-semibold">Rs. ${item.price.toFixed(2)}</p>
                    
                    <!-- Images -->
                    <div class="flex space-x-4 mt-4">
                    ${item.images.map(img => `<img src="${img}" class="w-20 h-20 rounded shadow" />`).join("")}
                    </div>
                    ...
                    <ul class="flex space-x-2">
                        ${item.sizes.map(size => `
                            <li>
                            <button class="size-option px-3 py-1 bg-gray-200 rounded hover:bg-gray-300" data-size="${size}">
                                ${size}
                            </button>
                            </li>
                        `).join("")}
                    ...
                    <ul class="flex space-x-2">
                        ${item.colors.map(color => `
                            <li>
                            <button class="color-option w-6 h-6 rounded-full bg-${color}-500" data-color="${color}">
                            </button>
                            </li>
                        `).join("")}
                    </ul>
                    ...
                    <!-- Add to Cart -->
                    <div class="mt-4">
                    <button id="add-to-cart-${item.id}" class="add-to-cart bg-red-500 text-white text-sm font-medium py-2 px-4 rounded-full hover:bg-red-600 transition-colors duration-300 animate-bounce shadow-md" data-id="${item.id}" data-name="${item.name}" data-price="${item.price}">ADD TO CART</button>
                    </div>
                `;
                ...
                // Add event listeners for selecting size and color
                ...
                document.querySelectorAll(".size-option").forEach(sizeButton => {
                    sizeButton.addEventListener("click", () => {
                        ...
                        document.querySelectorAll(".size-option").forEach(btn => btn.classList.remove("bg-blue-300"));
                        sizeButton.classList.add("bg-blue-300");
                    });
                });
                ...
                document.querySelectorAll(".color-option").forEach(colorButton => {
                    colorButton.addEventListener("click", () => {
                        ...
                        document.querySelectorAll(".color-option").forEach(btn => btn.classList.remove("ring-2", "ring-black"));
                        colorButton.classList.add("ring-2", "ring-black");
                    });
                });
    ...
    function addToCart(item, size, color) {
        const existingItem = cart.find(cartItem => cartItem.id === item.id && cartItem.size === size && cartItem.color === color);
        if (existingItem) {
            existingItem.quantity += 1;
        } else {
            cart.push({ ...item, size, color, quantity: 1 });
        }
    ...
    document.querySelectorAll("[id^='close-modal']").forEach(button => {
        button.addEventListener("click", (e) => {
            ...
            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                ...
                <div class="flex justify-center mt-4">
                <button class="see-more bg-blue-500 text-white text-sm font-medium py-2 px-4 rounded-full hover:bg-blue-600 transition-colors duration-300" data-id="77">
                    See More
                    </button>
                </div>
                </a>
